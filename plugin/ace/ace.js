  ...
  (function () {
    ...
    function extend(o) {
      for (var i = 1; i < arguments.length; i++)
        for (var key in arguments[i]) o[key] = arguments[i][key];
      return o;
    }
    function mkEditor(iframe) {
      ...
      if (mode) {
        d.write(
          "<script src='https://cdnjs.cloudflare.com/ajax/libs/ace/1.2.7/mode-" +
            mode +
      ...
      iframe.onload = function () {
        ...
        // Configuration
        if (aceConf.theme) editor.setTheme(aceConf.theme);
        if (aceConf.mode) editor.getSession().setMode(aceConf.mode);
        ...
        // Events
        if (options.oninit) options.oninit.call(editor, editor);
